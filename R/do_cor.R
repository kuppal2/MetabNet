...
max.cor.num,net_node_colors,net_node_shapes,net_edge_colors,net_legend,netrandseed=555,num_nodes=6, plots.width=2000,plots.height=2000,plots.res=300){
    ...
    NAs_present<-which(is.na(data_m_fc_withfeats)==TRUE)
    
    print("Number of NAs")
    print(length(NAs_present))
	...
	num_samp_matrix<- matrix(nrow=dim(pearson_res)[1],ncol=dim(pearson_res)[2],dim(data_m_fc_withfeats[,-c(1:2)])[2])
	
    if(length(NAs_present)>0){
        
        good_samp_perrow<-apply(data_m_fc_withfeats[,-c(1:2)],1,function(x){
            
            
            
            x1<-which(is.na(x)==FALSE)
            return(x1)
	...
	num_samp_matrix<-sapply(1:dim(num_samp_matrix)[1],function(k){
	
			#x<-lapply(1:length(x),function(j)
			
			x<-rep(length(good_samp_perrow[[k]]),length(good_samp_perrow))
			for(j in k:length(good_samp_perrow))
			{
			
			
				check_x<-good_samp_perrow[[j]]
				
				check_y<-good_samp_perrow[[k]]
				common_xy<-intersect(check_x,check_y)
				
				x[j]<-length(common_xy) #min(num_samp_perrow[j],num_samp_perrow[1])
	...
	num_samp_matrix[upper.tri(num_samp_matrix,diag=TRUE)]<-t(num_samp_matrix)[upper.tri(num_samp_matrix,diag=TRUE)]
    
    }
    #save(num_samp_matrix,file="num_samp_matrix.Rda")
	#num_samp_matrix<-ldply(num_samp_matrix,rbind)
        ...
        pearson_res<-round(pearson_res,3)
        #save(pearson_res,file="pearson_res.Rda")
       ...
       #print(pearson_res[1:3,1:5])
        ...
        if(is.na(subindex[1])==FALSE){
            goodfeats<-data_m_fc_withfeats[subindex,]
                ...
                num_samp_matrix<-num_samp_matrix[subindex,] 
        ...
        if(is.na(targetindex[1])==FALSE){
        	...
        	num_samp_matrix<-num_samp_matrix[,targetindex] 
	   }
            ...
            if(is.na(subindex[1])==FALSE){
         	...
         	num_samp_matrix<-num_samp_matrix[,-c(subindex)]       
            }
        }
    ...
    num_samp_vec<-as.vector(num_samp_matrix)
    
    pearson_pvalue<-parLapply(cl,1:length(pearson_resvec),getcorpvalue, pearson_resvec=pearson_resvec,num_samp_vec=num_samp_vec)
    stopCluster(cl)
    ...
    #print("here 3")
          ...
          rnames_mat<-rownames(cormat)
          cnames_mat<-colnames(cormat)
	...
	if(FALSE){ 
	 ...
	 complete_pearsonpvalue_mat<- complete_pearsonpvalue_mat[-which(is.na(sum_check1)==TRUE),]
	  ...
	  goodfeats<-goodfeats[,-which(is.na(sum_check1)==TRUE)]
          ...
          rownames(complete_pearsonpvalue_mat)<-cnames_mat
          colnames(complete_pearsonpvalue_mat)<-rnames_mat
          
          
          fname<-paste("correlation_pvalues.txt",sep="")
          write.table(complete_pearsonpvalue_mat,file=fname,sep="\t",row.names=TRUE)
          
          rownames(complete_pearsonqvalue_mat)<-cnames_mat
          colnames(complete_pearsonqvalue_mat)<-rnames_mat
		...
		nonsig_vs_fdr0.05_pearson_mat_bool_filt<-replace(as.matrix(nonsig_vs_fdr0.05_pearson_mat_bool_filt),which(is.na(nonsig_vs_fdr0.05_pearson_mat_bool_filt)==TRUE),0)
          ...
          #print(length(rnames))
                  ...
                  if(length(which(check_cor>=abs.cor.thresh))>0){
                      ...
                      #v5.1.2
                      net_result<-try(network(mat=as.matrix(nonsig_vs_fdr0.05_pearson_mat_bool_filt_1), threshold=abs.cor.thresh,X.names = NULL, Y.names = NULL, keep.var = TRUE,color.node = net_node_colors,shape.node = net_node_shapes,
                      color.edge = net_edge_colors,lty.edge = c("solid", "solid"), lwd.edge = c(1, 1),show.edge.labels = FALSE, interactive = FALSE),silent=TRUE)
                      ...
                      if(is(net_result,"try-error")){
                          ...
                          #6.1 or v5.2
                          net_result<-try(network(mat=as.matrix(nonsig_vs_fdr0.05_pearson_mat_bool_filt_1), threshold=abs.cor.thresh,row.names = TRUE, col.names = TRUE, block.var.names = TRUE,color.node = net_node_colors,shape.node = net_node_shapes,
                          ...
                          if(is(net_result,"try-error")){
                           ...
                           # print("here")
                           net_result<-try(network(mat=as.matrix(nonsig_vs_fdr0.05_pearson_mat_bool_filt_1), cutoff=abs.cor.thresh,row.names = TRUE, col.names = TRUE, block.var.names = TRUE,color.node = net_node_colors,shape.node = net_node_shapes,
                           color.edge = net_edge_colors,lty.edge = "solid", lwd.edge = 1,show.edge.labels = FALSE, interactive = FALSE,cex.node.name=0.7),silent=TRUE)
              ...
              if(length(nonsig_vs_fdr0.05_pearson_mat_bool_filt)==dim(goodfeats)[1]){
                  ...
                  stop("No significant correlations found.")
              }
          }
